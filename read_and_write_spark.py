...
def create_separate_dataframes() -> dict:
    ...
    for filename in os.listdir(directory):
        ...
        for new_files in os.listdir(new_directory):
            ...
            dataframes[my_path] = pd.read_csv(f, names=['ts_min_bignt', columns[
                count]])  # Sample key: 656_co2. Dataframe columns: ts_min_bignt, co2
            count += 1
            count2 += 1
...
def create_main_dataframe(separate_dataframes: dict):
    """
    Concat all per-room dataframes vertically. Creates final dataframe.
    """
    ...
    df = df.sort_values(
        'ts_min_bignt')  # All data is sorted according to ts_min_bignt. We want it to stream according to timestamp.
...
def write_main_dataframe(df):
    ...
    """
    df.to_csv('dataset/sensors.csv', index=False)


if __name__ == "__main__":
    sd = create_separate_dataframes()
    md = create_main_dataframe(sd)
    write_main_dataframe(md)
